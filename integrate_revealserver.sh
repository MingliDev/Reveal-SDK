...
echo "Reveal Server framework load trigger configuration: ${load_trigger}"
...
if [ "${CONFIGURATION}" != "${load_trigger}" ]; then
    echo "Going to remove Reveal Server framework from ${TARGET_NAME}"
    ...
    echo "Expected Reveal Server framework path: ${bundled_reveal_server}"

    if [ -e "${bundled_reveal_server}" ]; then
        echo "Reveal Server framework exists, removing"
        ...
        if [ -L "${bundled_reveal_server}" ]; then
            echo "Reveal Server framework symlink exists, but points to nowhere"
        else
            echo "Reveal Server framework not found"
        fi

        echo "Make sure you placed Reveal Server integration phase in Xcode below Embed Frameworks phase (or as very last phase)"
    fi
